...
var config = {
  ...
  var imageBecRef = firebase.database().ref("website/student/graduate/activity").child(sessionStorage.getItem("activityId")).child('activity_image');
...
var count=0;
  ...
  var nameBecRef = firebase.database().ref("website/student/graduate/activity").child(sessionStorage.getItem("activityId")).child('activity_name');
  nameBecRef.on('value',snap=>{
    $('#nameActivity').text(snap.val());
  });

  var detailBecRef = firebase.database().ref("website/student/graduate/activity").child(sessionStorage.getItem("activityId")).child('activity_detail');
  detailBecRef.on('value',snap=>{
    $('#detailActivity').text(snap.val());
  });
  var dateStart;
  var dateStartBecRef = firebase.database().ref("website/student/graduate/activity").child(sessionStorage.getItem("activityId")).child('activity_start');
  dateStartBecRef.on('value',snap=>{
    dateStart = snap.val();
  });

  var dateEndBecRef = firebase.database().ref("website/student/graduate/activity").child(sessionStorage.getItem("activityId")).child('activity_end');
