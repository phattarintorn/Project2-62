$(document).ready(function(){
  ...
  var config = {
    ...
    firebase.initializeApp(config);
      ...
      var numCountPortGraduate = firebase.database().ref('statistic/portfolio_graduate_page').child(yearViewPage).child(monthViewPage).child('count');
      numCountPortGraduate.on('value',snap=>{
        n = snap.val();
        ...
        if(countPage==1){
          firebase.database().ref('statistic/portfolio_graduate_page').child(yearViewPage).child(monthViewPage).update(countView);
        }
...
// ======================================================= Get Portfolio =======================================================
  portGraRef.on("child_added",snap => {
    var key = snap.key;
  ...
  $('#showPort').on('click','.detail',function(e){
    ...
    var dbDetail = portGraRef.child(id).child('port_detail');
    ...
    var dbType = portGraRef.child(id).child('port_type');
    ...
    var dbYear = portGraRef.child(id).child('port_years');
    ...
    var dbImage = portGraRef.child(id).child('port_image');
