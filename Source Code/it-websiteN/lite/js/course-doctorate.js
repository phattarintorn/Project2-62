$(document).ready(function(){
  ...
  var numCountCourseDoctorate = firebase.database().ref('statistic/course_doctorate_page').child(yearViewPage).child(monthViewPage).child('count');
  numCountCourseDoctorate.on('value',snap=>{
    n = snap.val();
  ...
  var rootRefCourses = topicRef.child("course/AddCourses").child('ปริญญาเอก');
  rootRefCourses.on("child_added",snap => {
   ...
   i=i+1;
    ...
    $('#Suggestion_work').append(
      ...
      "</tr>"
        ...
        if(i==1){
                ...
                var rootRefBranch = topicRef.child("course/AddCourses").child('ปริญญาเอก').child(snapkey);
                ...
                $('#TitlePlan').text(snapkey);
                     ...
                     var DownloadPlan = topicRef.child("course/AddStudyplan").child('ปริญญาเอก').child(snapkey).child('Studyplan').child('file');
          ...
          rootRefBranch.on("child_added",snap => {
           ...
           $('#Branch_work').append(
              ...
              var Download = topicRef.child("course/AddDownload").child('ปริญญาเอก').child(snapkey);
          $('#Download_work').empty();
    ...
    $('#Suggestion_work').on('click','.btn-edit-expert',function(){
      ...
      var rootRefBranch = topicRef.child("course/AddCourses").child('ปริญญาเอก').child(key);
      ...
      $('#TitlePlan').text(key);
           ...
           var DownloadPlan = topicRef.child("course/AddStudyplan").child('ปริญญาเอก').child(key).child('Studyplan').child('file');
           DownloadPlan.on('value',snap => {
              $('#downloadPlan').attr("href",snap.val());
            });
...
rootRefBranch.on("child_added",snap => {
 ...
 $('#Branch_work').append(
    ...
    /*------------------------  edit Dowload (2555) -----------------------------------*/
    var Download = topicRef.child("course/AddDownload").child('ปริญญาเอก').child(key);
$('#Download_work').empty();
 Download.on("child_added",snap => {
   var snapkey = snap.key;
   var topic = snap.child('topic').val();
